...
def get_metrics(output, annotation, ini_cls=2, num_cls=None):
    
    out = output[0,0]
    anno = annotation[0,0]
    
    if num_cls is None:
        num_cls = anno.max().item()
    
    J = 0.0
    F = 0.0
    count = 0
    
    for classid in range(ini_cls, num_cls+1):
        obj_true = (anno == classid).cpu().numpy()
        obj_pred = (out == classid).cpu().numpy() 
        J += eval_iou(obj_true, obj_pred)
        F += eval_boundary(obj_true, obj_pred)
        count += 1
            
        
    
    return J/max(count, 1), F/max(count, 1)
