...
from isec_py.my_canny import my_canny
from skimage.exposure import rescale_intensity
from scipy.ndimage.filters import convolve

class isec():
    
    def __init__(self, f_order=11, nit=5, at=20, display=False):
    ...
    def segment(self, img):
        ...
        for it in range(self.nit):
            ...
            # Update treshold
            k = math.exp(0.02*it)/10
            
            # Select edges
            ed = edge_detector.get_edges(high_threshold=k, low_threshold=0.1*k, it=it)
            ...
            # thin the edges expanded by the filter
            ed_den = thin_edges(ed_den, S)
            ...
            im_shell, _ = shell_kernel(ed_den)
            
            # Store the segments
            segments = (segments + im_shell)>0
            
        # post-processing
        segments = area_closing(segments, area_threshold=self.at, connectivity=1)
        segments = thin_edges(segments, self.f_order)
